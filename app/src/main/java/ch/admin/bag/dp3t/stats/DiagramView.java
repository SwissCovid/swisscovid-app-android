...
public class DiagramView extends View {
	...
	public int getTotalTheoreticWidth() {
		return Math.round(history.size() * WIDTH_BAR * dp + (history.size() - 1) * PADDING_BAR * dp + getMondayPadding(true) +
				getMondayPadding(false));
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.translate(-canvasXTranslation + getMondayPadding(true), 0);
	...
	private float getMondayPadding(boolean left) {
		...
		boolean isEntryMonday = cal.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY;
		if (isEntryMonday) {
